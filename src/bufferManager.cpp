...
Page BufferManager::getPageLoad(string tableName, int pageIndex)
{
    logger.log("BufferManager::getPage");
    string pageName = "../data/temp/"+tableName + "_Page" + to_string(pageIndex);
    // if (this->inPool(pageName))
...
{
    /*NOTE - first create a page and then write on that page*/
...
{
    /*NOTE - first create a page and then write on that page*/
    logger.log("BufferManager::clearBuffer");
    pages.clear();
}
...
//-----Matrix---------
void BufferManager::writePageMatrix(string matrixName, pair<int,int> pageIndex, vector<vector<int>> rows,int rowCount)
{
    /*NOTE - first create a page and then write on that page*/
    logger.log("BufferManager::writePage");
...
Page BufferManager::getPageMatrix(string matrixName, pair<int,int> pageIndex)
{
    logger.log("BufferManager::getPage");
    string pageName = "../data/temp/"+matrixName + "_matPage" + "_" + to_string(pageIndex.first) + "_" + to_string(pageIndex.second);
    if (this->inPool(pageName))
        return this->getFromPool(pageName);
    else
...
Page BufferManager::insertIntoPoolMatrix(string matrixName, pair<int,int>  pageIndex)
{
    logger.log("BufferManager::insertIntoPool");
...
/**
 * @brief If the page is present in the pool, then this function deletes the
 * page.
 *
 * @param pageName 
 * @return Page 
...
void BufferManager::deleteFromPool(string pageName)
{
    ...
    for (auto &page : this->pages)
        if (pageName == page.pageName)
        {
            page.pageName="INVALID_PAGE";
            logger.log("BufferManager::deleteFromPool::PageDeleted");
        }
}