...
class BufferManager{
    ...
    Page getPageLoad(string tableName, int pageIndex);
    void clearBuffer();

    
    //-----------------------------Our Code-------------------------------------------
    void writePageMatrix(string matrixName, pair<int,int> pageIndex, vector<vector<int>> rows,int rowCount);
    Page getPageMatrix(string MatrixName, pair<int,int> pageIndex);
    Page insertIntoPoolMatrix(string matrixName, pair<int,int> pageIndex);
